...
# random.choice(l) renvoie aleatoirement un elem de la liste l
...
class Grid:
    ...
    # la fonction remplit aléatoirement une case vide après chaque mouvement
    def case_aleatoire(self):
        c = random.choice(self.cases_vides()) # retourne un couple ligne et colonne
        i = c[0]
        j = c[1]
        self.cells[i][j] = 2 if random.random() < 0.9 else 4


    # la fonction retourne la liste des cases vides
    ...
    # la fonction génère une grille vide
    ...
    # la fonction cherche si une case contient 2048 ou un nombre plus grand
    def found_2048(self):
        for i in range(self.size):
            for j in range(self.size):
